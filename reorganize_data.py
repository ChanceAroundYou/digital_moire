...
from utils.preprocess.folder import copy_with_hash_check, is_date, is_project_id
from utils.preprocess.rename import rename_folder


def copy_scan_files(scan_dir, project_dest_dir, date, scan_type):
    """
    ...
    file_mappings = {
        "fuse.ply": f"{scan_type}_fuse_{date}.ply",
        "fuse_mesh.ply": f"{scan_type}_fuse_mesh_{date}.ply",
    }
...
def process_project(date_dir_path, project_id, dest_dir):
    ...
    """
    date = os.path.basename(date_dir_path)
    project_path = os.path.join(date_dir_path, project_id)
    rename_folder(project_path)
    ...
    for scan_name in os.listdir(project_data_dir):
        ...
        if scan_name.startswith("STDD_"):
            ...
            copy_scan_files(scan_dir_path, project_dest_dir, date, "STDD")
        elif scan_name.startswith("STD_"):
            logger.info(f"Found STD scan folder: {scan_name}")
            copy_scan_files(scan_dir_path, project_dest_dir, date, "STD")
        elif scan_name.startswith("ATR_"):
            logger.info(f"Found ATR scan folder: {scan_name}")
            copy_scan_files(scan_dir_path, project_dest_dir, date, "ATR")
